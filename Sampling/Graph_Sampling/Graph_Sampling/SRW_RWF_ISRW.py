...
class SRW_RWF_ISRW:
    ...
    def random_walk_sampling_simple(self,complete_graph, nodes_to_sample, node_seed):
        complete_graph = nx.convert_node_labels_to_integers(complete_graph, 0, 'default', True)
        index_of_first_random_node = 0
        for n, data in complete_graph.nodes(data=True):
        #    complete_graph.node[n]['id'] = n
            if node_seed == data[True]:
                index_of_first_random_node = n
                break

        
        nr_nodes = len(complete_graph.nodes()) -1
        upper_bound_nr_nodes_to_sample = nodes_to_sample
        #index_of_first_random_node = random.randint(0, nr_nodes)
        curr_node_originalName = complete_graph.node[index_of_first_random_node][True]
        sampled_graph = nx.Graph()
        sampled_graph.add_node(curr_node_originalName)
        ...
        while sampled_graph.number_of_nodes() != upper_bound_nr_nodes_to_sample:
            ...
            chosen_node_originalName = complete_graph.node[chosen_node][True]
            sampled_graph.add_node(chosen_node_originalName)
            sampled_graph.add_edge(curr_node_originalName, chosen_node_originalName)
            curr_node = chosen_node
            curr_node_originalName = chosen_node_originalName
            iteration = iteration+1

            if iteration % self.T == 0:
                if ((sampled_graph.number_of_edges() - edges_before_t_iter) < self.growth_size):
                    curr_node = random.randint(0, nr_nodes)
                    curr_node_originalName = complete_graph.node[curr_node][True]
                edges_before_t_iter = sampled_graph.number_of_edges()
        return sampled_graph

    def random_walk_sampling_with_fly_back(self,complete_graph, nodes_to_sample, fly_back_prob, node_seed):
        complete_graph = nx.convert_node_labels_to_integers(complete_graph, 0, 'default', True)
    ...
    def random_walk_induced_graph_sampling(self, complete_graph, nodes_to_sample, node_seed):
        complete_graph = nx.convert_node_labels_to_integers(complete_graph, 0, 'default', True)
        # giving unique id to every node same as built-in function id
        index_of_first_random_node = 0
        for n, data in complete_graph.nodes(data=True):
            complete_graph.node[n]['id'] = data[True]
            if node_seed == data[True]:
                index_of_first_random_node = n
